...
riskCurve <- function(formula, bsm, tx, data, hinge=FALSE, weights=NULL, saveFile=NULL, saveDir=NULL){
  ...
  if (!is.null(weights)){ 
    if (is.character(weights)){
      colnames(data)[colnames(data2)==weights] <- "weights"
    } else {
  ...
  nTCases2new <- nTCases * nTControlsI / nTControls
  ...
  if (hinge){
    ...
    # in each study group separately, compute the change point for the association of S with Y
    ...
    # use their minimum as the change point in the below specified GLMs
